  ...
  function _link_body( o )
  {
    ...
    if( o.sync )
    {
      ...
      try
      {
        ...
        if( tempPath )
        {
          self.filesDelete({ filePath : tempPath, verbosity : 0 });
          tempPath = null;
        }
    ...
    function tempRename()
    {
      ...
      tempPath = tempNameMake( o2.dstPath );
      if( self.statRead({ filePath : tempPath }) )
      self.filesDelete( tempPath );
      self.fileRenameAct
      ({
        dstPath : tempPath,
        srcPath : o.dstPath,
    ...
    function validateSize()
    {
      ...
      // if( !Config.debug )
      // return;
      // let srcStat = self.statResolvedRead({ filePath : o.srcPath, resolvingSoftLink : 1, resolvingTextLink : 1 });
      // if( !srcStat )
      // return;
      // let dstStat = self.statResolvedRead({ filePath : o.dstPath, resolvingSoftLink : 1, resolvingTextLink : 1 });
      // if( !dstStat )
      // return;
      // _.assert( !!dstStat );
    ...
    function end( r )
    {
      ...
      result = _.Consequence().give( r );
      _.assert( !!o.sync || _.consequenceIs( result ) );
...
function fileCopy_functor()
{
  ...
  function _onBeforeRaname( o )
  {
    ...
    let isDir = self.isDir({ filePath : o.srcPath, resolvingSoftLink : 0, resolvingTextLink : 0 })
    if( isDir )
    {
      debugger;
...
// }
//
// filesAreSoftLinked.pre = filesAreSoftLinked_pre;
// filesAreSoftLinked.body = filesAreSoftLinked_body;
//
// var having = filesAreSoftLinked.having = Object.create( filesAreSoftLinked_body.having );
// having.driving = 0;
// having.aspect = 'entry';

let filesAreSoftLinked = _.routineFromPreAndBody( filesAreSoftLinked_pre, filesAreSoftLinked_body );
...
function filesAreHardLinked_pre( routine, args )
{
  let self = this;
  _.assert( arguments.length === 2, 'Expects exactly two arguments' );
  if( args.length !== 1 || ( !_.arrayIs( args[ 0 ] ) && !_.argumentsArrayIs( args[ 0 ] ) ) )
...
function filesAreHardLinked_body( files )
{
  let self = this;
  ...
  let statFirst = self.statResolvedRead( files[ 0 ] );
  if( !statFirst )
  return false;
...
var having = filesAreHardLinked_body.having = Object.create( filesAreHardLinkedAct.having );
having.driving = 0;
having.aspect = 'body';
...
// }
//
// filesAreHardLinked.pre = filesAreHardLinked_pre;
// filesAreHardLinked.body = filesAreHardLinked_body;
//
// var having = filesAreHardLinked.having = Object.create( filesAreHardLinked_body.having );
// having.driving = 0;
// having.aspect = 'entry';

let filesAreHardLinked = _.routineFromPreAndBody( filesAreHardLinked_pre, filesAreHardLinked_body );
